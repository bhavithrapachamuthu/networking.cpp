...
#define PACKET_SEARCH 12
#define PACKET_DELETE 13
...
void viewAll(SOCKET client){
    if(!sendPacket(client,PACKET_VIEW_ALL,0,FRAME_SINGLE,NULL)){
        printf("Failed to send view All request\n");
        return;
    }
    while(true){
        Packet packet;
        ...
        if(packetID == PACKET_RESULT){
            ...
            if(strcmp(result,"END") == 0){
                printf("\nALL DATA RECEIVED\n");
                printf("\nView all completed successfully\n");
                break;
            }
        }
        if(packetID == PACKET_EMPLOYEE_ID)
        {
            ...
            int empId;
            memcpy(&empId,packet.data,sizeof(int));
            printf("Employee Id: %d\n",empId);
        }
        else if(packetID == PACKET_CUSTOMER_ID){
            int cusId;
            memcpy(&cusId,packet.data,sizeof(int));
            printf("Customer Id: %d\n",cusId);
        }
        else if(packetID == PACKET_SALE_AMOUNT){
            float amount;
